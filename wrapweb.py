    ...
    if ('account' not in parameters.keys() or not parameters['account']) and (
        ...
        def myfunc():
            ...
            if request.method == 'POST':
                ...
                if parameters['input']['files']:
                    args.append(app.config['UPLOAD_FOLDER'])
                result = function(*args)
                if isinstance(result, tuple):
                    result = list(result)
                else:
                    result = [result]
            return render_template('index.html', parameters=parameters, url=url_site, result=result,
                                   l=len(parameters['output']))
